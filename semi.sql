...
select * from tbl_purchase_status;
select * from tbl_prod_status;
update tbl_prod_status set status_name='삭제 완료' where status_code='S99';
commit;



SELECT FILE_PATH, FILE_NO
FROM TBL_FILE
WHERE PRODUCT_NO = 'P2505230042'
ORDER BY FILE_NO ASC;

SELECT P.ORDER_NO, P.PRODUCT_NO, P.SELLER_MEMBER_NO, P.BUYER_MEMBER_NO, P.DELIVERY_ADDR, P.DELIVERY_FEE, P.ORDER_AMOUNT, P.PG_PROVIDER, P.PG_TRANSACTION_ID, P.DEAL_DATE, P.PURCHASE_STATUS_CODE, PROD.PRODUCT_NAME, PROD.PRODUCT_PRICE, SELLER.MEMBER_NICKNAME AS SELLER_NICKNAME, PS.STATUS_NAME AS PURCHASE_STATUS_NAME, (SELECT FL.FILE_PATH FROM (SELECT F.FILE_PATH, F.FILE_NO FROM TBL_FILE F WHERE F.PRODUCT_NO = P.PRODUCT_NO ORDER BY F.FILE_NO ASC) FL WHERE ROWNUM = 1) AS THUMBNAIL_PATH FROM TBL_PURCHASE P JOIN TBL_PROD PROD ON P.PRODUCT_NO = PROD.PRODUCT_NO JOIN TBL_MEMBER SELLER ON P.SELLER_MEMBER_NO = SELLER.MEMBER_NO JOIN TBL_PURCHASE_STATUS PS ON P.PURCHASE_STATUS_CODE = PS.PURCHASE_STATUS_CODE WHERE P.BUYER_MEMBER_NO = 'M2505220027' ORDER BY P.DEAL_DATE DESC;
SELECT P.ORDER_NO, P.PRODUCT_NO AS PURCHASE_PRODUCT_NO, P.SELLER_MEMBER_NO, P.BUYER_MEMBER_NO, P.DELIVERY_ADDR, P.DELIVERY_FEE, P.ORDER_AMOUNT, P.PG_PROVIDER, P.PG_TRANSACTION_ID, P.DEAL_DATE, P.PURCHASE_STATUS_CODE, PROD.PRODUCT_NAME, PROD.PRODUCT_PRICE, SELLER.MEMBER_NICKNAME AS SELLER_NICKNAME, PS.STATUS_NAME AS PURCHASE_STATUS_NAME, (SELECT FL.FILE_PATH FROM (SELECT F.FILE_PATH, F.FILE_NO FROM TBL_FILE F WHERE F.PRODUCT_NO = PROD.PRODUCT_NO ORDER BY F.FILE_NO ASC) FL WHERE ROWNUM = 1) AS THUMBNAIL_PATH FROM TBL_PURCHASE P JOIN TBL_PROD PROD ON P.PRODUCT_NO = PROD.PRODUCT_NO JOIN TBL_MEMBER SELLER ON P.SELLER_MEMBER_NO = SELLER.MEMBER_NO JOIN TBL_PURCHASE_STATUS PS ON P.PURCHASE_STATUS_CODE = PS.PURCHASE_STATUS_CODE WHERE P.BUYER_MEMBER_NO = 'M2505220027' ORDER BY P.DEAL_DATE DESC;
...
-- 20250523 남후님 요청 사항
-- 마지막 SQL 수정문입니다. 이제 진짜 끝 더 추가안합니다
...
ADD REPORT_DETAIL VARCHAR2(300)  -- 신고 내용 (한글 약 100자 내외)
;

commit;

-- 택배사 정보 추가 20250523 임동주

ALTER TABLE TBL_PURCHASE
ADD (
    DELIVERY_COMPANY_CODE VARCHAR2(10), -- 택배사 코드 (TBL_DELIVERY_COMPANY 테이블과 연결될 외래키)
    DELIVERY_COMPANY_NAME VARCHAR2(100), -- 택배사 명 (만약 코드를 사용하지 않고 직접 입력받는다면)
    TRACKING_NUMBER VARCHAR2(50),    -- 송장번호
    SHIPMENT_DATE DATE               -- 발송일 (선택 사항, 발송 정보 저장 시 SYSDATE로 기록 가능)
);
...
CREATE TABLE TBL_DELIVERY_COMPANY (
    COMPANY_CODE VARCHAR2(10) PRIMARY KEY, -- 택배사 코드 (예: 'CJ', 'POST', 'HANJIN')
    COMPANY_NAME VARCHAR2(100) NOT NULL    -- 택배사 명 (예: 'CJ대한통운', '우체국택배')
);


INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('CJ', 'CJ대한통운');
INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('POST', '우체국택배');
INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('HANJIN', '한진택배');
INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('LOTTE', '롯데택배');
INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('ROZEN', '로젠택배');
INSERT INTO TBL_DELIVERY_COMPANY (COMPANY_CODE, COMPANY_NAME) VALUES ('ETC', '기타'); -- "기타" 항목에 대한 코드
COMMIT; -- 데이터베이스에 변경사항 최종 반영

commit;

SELECT COMPANY_CODE, COMPANY_NAME FROM TBL_DELIVERY_COMPANY;