...
<%-- postDate가 String이므로 fmt:formatDate는 사용 안 함 --%>
...
<head>
    ...
    <style>
        ...
        *, *:before, *:after { box-sizing: inherit; }
        ...
        .review-detail-container { max-width: 960px; margin: 20px auto; background-color: #fff; padding: 25px 30px; box-shadow: 0 2px 15px rgba(0,0,0,0.08); border-radius: 8px; }
        .review-header { margin-bottom: 25px; border-bottom: 1px solid #e9ecef; padding-bottom: 20px; }
        .review-header h2 { margin: 0 0 12px 0; font-size: 2.2em; font-weight: 700; color: #212529; }
        .review-info { font-size: 0.95em; color: #495057; }
        .review-info span { margin-right: 18px; line-height: 1.5; }
        .review-info .category-display { background-color: #e6f7ff; color: #007bff; padding: 4px 10px; border-radius: 4px; font-weight: 500; }
        .review-content { margin-bottom: 30px; line-height: 1.8; min-height: 100px; white-space: pre-wrap; font-size: 1.1em; color: #343a40; }
        .post-images-slider { margin: 0 auto 30px auto; max-width: 100%; }
        .post-images-slider .slick-slide { padding: 0 5px; }
        .post-images-slider .slick-slide div { outline: none; }
        .post-images-slider img { width: 100%; max-height: 600px; object-fit: contain; border-radius: 6px; display: block; margin: auto; }
        .no-images-message { text-align: center; color: #6c757d; margin: 20px 0; padding: 30px; border: 1px dashed #ced4da; border-radius:5px; background-color: #f8f9fa;}
        .slick-prev, .slick-next { width: 45px; height: 45px; z-index: 10; background-color: rgba(250, 250, 250, 0.6); border-radius: 50%; transition: background-color 0.2s ease; }
        .slick-prev:hover, .slick-next:hover { background-color: rgba(250, 250, 250, 0.9); }
        .slick-prev { left: 10px; } .slick-next { right: 10px; }
        .slick-prev:before, .slick-next:before { font-family: 'Material Icons'; font-size: 30px; color: #555; opacity: 0.7; line-height: 1; }
        .slick-prev:hover:before, .slick-next:hover:before { opacity: 1; color: #000; }
        .slick-prev:before { content: "chevron_left"; } .slick-next:before { content: "chevron_right"; }
        .slick-dots { bottom: 10px; }
        .slick-dots li button:before { font-size: 10px !important; color: #adb5bd !important; }
        .slick-dots li.slick-active button:before { color: #007bff !important; }
        .comments-section { border-top: 1px solid #e9ecef; padding-top: 25px; }
        .comments-section h3 { font-size: 1.6em; color: #212529; margin-bottom: 20px; font-weight: 500; }
        .comment-input-area { display: flex; align-items: stretch; margin-bottom: 25px; }
        .comment-input-area textarea { flex-grow: 1; padding: 12px 15px; border: 1px solid #ced4da; border-radius: 4px 0 0 4px; margin-right: 0; resize: vertical; min-height: 80px; font-family: 'Noto Sans KR', sans-serif; font-size: 1em; }
        .comment-input-area button { flex-shrink: 0; padding: 0 20px; background-color: #007bff; color: white; border: none; border-radius: 0 4px 4px 0; cursor: pointer; font-weight: 500; font-size: 1em; line-height: normal; }
        .comment-input-area button:hover { background-color: #0056b3; }
        .comment-login-prompt { text-align: center; width: 100%; color: #495057; padding: 15px; background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 4px; }
        .comment-login-prompt a { color:#007bff; text-decoration:none; font-weight:bold; }
        ul.comment-list { list-style-type: none; padding-left: 0; margin-top: 0; }
        li.comment-item { border-bottom: 1px dashed #e9ecef; padding: 15px 0; }
        li.comment-item:last-child { border-bottom: none; }
        .comment-meta { display: flex; align-items: center; margin-bottom: 8px; }
        .comment-meta .comment-nickname { font-weight: 700; font-size: 1.05em; margin-right: 12px; color: #212529; }
        .comment-meta .comment-date { font-size: 0.85em; color: #6c757d; }
        p.comment-content { margin-top: 5px; margin-bottom: 10px; white-space: pre-wrap; line-height: 1.7; color: #343a40; font-size: 1em; }
        div.input-item { display: none; margin-top: 8px; }
        div.input-item textarea.edit-comment-textarea { width: 100%; min-height: 60px; padding: 10px 12px; border: 1px solid #ced4da; border-radius: 4px; resize: vertical; font-family: 'Noto Sans KR', sans-serif; font-size: 0.95em; margin-bottom: 5px; }
        .comment-actions { text-align: right; margin-top: 5px; }
        .comment-actions a { font-size: 0.85em; padding: 5px 10px; margin-left: 8px; background-color: #f8f9fa; border: 1px solid #ced4da; border-radius: 4px; cursor: pointer; color: #495057; text-decoration: none; }
        .comment-actions a:hover { background-color: #e2e6ea; border-color: #adb5bd; }
        .edit-comment-area { display: flex; align-items: stretch; margin-top: 10px; }
        .edit-comment-area textarea { flex-grow: 1; padding: 10px 12px; border: 1px solid #ced4da; border-radius: 4px 0 0 4px; margin-right: 0; resize: vertical; min-height: 60px; font-family: 'Noto Sans KR', sans-serif; font-size: 0.95em;}
        .edit-comment-area button { flex-shrink: 0; padding: 0 15px; border: none; cursor: pointer; font-weight: 500; font-size: 0.9em; line-height: normal;}
        .edit-comment-area button.save-btn { background-color: #28a745; color: white; border-radius: 0; }
        .edit-comment-area button.cancel-btn { background-color: #6c757d; color: white; border-radius: 0 4px 4px 0; }
        ...
        .back-to-list, .post-action-btn { display: inline-block; padding: 10px 20px; color: white; text-decoration: none; border-radius: 5px; font-weight: 500; font-size: 0.95em; margin-left: 10px; border: none; cursor:pointer; }
        .back-to-list { background-color: #007bff;}
        ...
        .post-actions-container { margin-top: 20px; padding-top: 20px; border-top: 1px solid #e9ecef; text-align: right; }
        .post-actions-container .edit-post-btn { background-color: #17a2b8; }
        .post-actions-container .edit-post-btn:hover { background-color: #138496; }
        .post-actions-container .delete-post-btn { background-color: #dc3545; }
        .post-actions-container .delete-post-btn:hover { background-color: #c82333; }
    </style>
</head>
<body>
                    ...
                    <span class="category-display">
                        ...
                        <c:choose>
                            <c:when test="${not empty category}">
                            	<c:out value="${category.larCategoryName} > ${category.midCategoryName} > ${category.categoryName}"/></c:when>
    ...
    <script>
        $(document).ready(function(){
            ...
            if ($slider.length > 0 && $slider.find('img').length > 0) {
                if (!$slider.hasClass('slick-initialized')) {
                    $slider.slick({ dots: true, infinite: true, speed: 500, slidesToShow: 1, adaptiveHeight: true, autoplay: true, autoplaySpeed: 3500, fade: true, cssEase: 'linear' });
                }
            }
        });
        ...
        function confirmDeletePost(stylePostNo) {
            swal({
                title: "게시글 삭제 확인", text: "이 게시글을 정말로 삭제하시겠습니까?\n삭제된 게시글은 복구할 수 없습니다.", icon: "warning",
                buttons: { cancel: { text: "취소", value: false, visible: true, closeModal: true }, confirm: { text: "삭제", value: true, visible: true, closeModal: true, className: "btn-danger" }},
                dangerMode: true,
            }).then(function(isConfirm) {
                if (isConfirm) {
                    let form = document.createElement('form');
                    form.method = 'POST';
                    form.action = '/review/delete'; // 예: ${pageContext.request.contextPath}/review/delete.do
                    let inputPostNo = document.createElement('input');
                    inputPostNo.type = 'hidden'; inputPostNo.name = 'stylePostNo'; inputPostNo.value = stylePostNo;
