...
/* 링크형 텍스트 스타일 */
.product-manage-links a, .product-manage-links .text-link-delete {
	color: var(--gray4);
	background: none;
	border: none;
	font-size: 13px;
...
.product-manage-links a:hover, .product-manage-links .text-link-delete:hover
	{
	color: var(--main3);
}
...
/* 답글 버튼 */
.comment-box-reply-btn, .comment-box-delete-btn, .comment-box-edit-btn {
	border: 1px solid var(--gray4);
...
.report-modal-container .modal-main {
  display: flex;
  flex-direction: column;
  gap: 20px;
}

.report-modal-container .input-wrap {
  margin-bottom: 16px;
}
.report-modal-container .modal-btn-area {
  margin-top: 12px;
}

/* 신고 모달 내부 textarea 스타일 조정 */
.report-modal-container textarea[name="reportDetail"] {
  height: 130px;         /* 기존보다 작게 */
...
/* 신고 모달 글자 수 표시용 */
.report-modal-container .char-count {
  text-align: right;
  font-size: 12px;
  color: var(--gray4);
  margin-top: 4px;
...
<body>
	<c:set var="isLoggedIn" value="${not empty sessionScope.loginMember}" />
	...
	<main class="container">
		...
		<section class="product-top">
			...
			<div class="product-summary">
				...
				<c:if
					test="${loginMember != null && (loginMember.memberNo eq product.memberNo || loginMember.memberId eq 'admin')}">
				...
				<div
					style="display: flex; align-items: center; gap: 8px; margin-bottom: 25px;">
					<c:choose>
						<c:when test="${product.statusCode eq 'S99'}">
							<c:if
								test="${loginMember != null and loginMember.memberId eq 'admin'}">
				...
				<div class="product-meta">
					<span>찜 <strong>${product.wishlistCount}</strong></span> | <span>조회수
						<strong>${product.readCount}</strong>
					</span> | <span>등록일 <strong><fmt:formatDate
								value="${product.enrollDate}" pattern="yyyy.MM.dd" /></strong></span>
				</div>

				<%-- 가격제안 여부(product.priceOfferYn == 'Y')일 때만 안내 문구 표시 --%>
				<c:if test="${product.priceOfferYn eq 'Y'}">
					<div class="price-offer-notice">
						<span class="material-icons">local_offer</span> 이 상품은 <strong>가격
							제안</strong>을 받을 수 있어요!
					</div>
				</c:if>
				...
				<div class="action-buttons"
					...
					<form id="wishlistForm" action="/wishlist/toggle" method="post">
						...
						<button type="submit" class="btn-secondary outline"
							id="wishlistBtn">
							<c:choose>
					...
					<!--  신고하기 버튼 -->
					<div class="action-buttons">
					  <button type="button" class="btn-warning" onclick="openReportModal()">신고하기</button>
					</div>

					<!--  신고 모달창 -->
					<div class="modal-bg report-modal-bg" id="reportModal">
					  <div class="report-modal-container">
					    <div class="modal-title">신고하기</div>
					    <span class="modal-close-btn" onclick="closeReportModal()">&times;</span>

							<form action="${pageContext.request.contextPath}/admin/reportInsert" method="post" onsubmit="return validateReportForm();">
								<div class="modal-main">

									<!-- 신고 사유 선택 -->
									<div class="input-wrap">
										<label class="input-title">신고 사유</label>
										<div class="select">
											<select name="reportCode" required>
												<option value="">-- 선택하세요 --</option>
												<c:forEach var="reason" items="${reportReasonList}">
													<option value="${reason.reportCode}">${reason.reportReason}</option>
												</c:forEach>
											</select>
										</div>
									</div>
									...
									<!-- 상세 내용 입력 -->
									<div class="input-wrap">
										<label class="input-title">신고 내용</label>
											<div class="input-item">
											  <textarea name="reportDetail" id="reportDetail" maxlength="100"
											    placeholder="신고 이유를 간단히 적어주세요 (최대 100자까지)" required></textarea>
											  <div class="char-count" id="reportDetailCount">0 / 100</div>
											</div>
									</div>

									<!-- 숨겨진 필드 -->
									<input type="hidden" name="reportedMemberNo" value="${sessionScope.loginMember.memberNo}"> 
								    <input type="hidden" name="productNo" value="${product.productNo}">
								...
								<div class="modal-btn-area">
									...
									<button type="submit" class="btn-primary">신고 제출</button>
								</div>
							</form>
						</div>
					</div>
...
--%>
		...
		<section class="related-products">
			...
			<button type="button" class="related-slide-btn left"
				id="relatedPrevBtn">&#10094;</button>
			<button type="button" class="related-slide-btn right"
				id="relatedNextBtn">&#10095;</button>
			...
			<div class="related-list" id="relatedListSlider">
				<c:forEach var="prod" items="${relatedProducts}">
					<c:if test="${prod.statusCode ne 'S99'}">
						<a
							href="${pageContext.request.contextPath}/product/detail?no=${prod.productNo}"
		...
		<section class="product-info-section">
			...
			<aside class="seller-info-box">
				...
				<div class="seller-links">
					<a
						href="/product/seller?memberNo=${not empty product.memberNo ? product.memberNo : 'M00000001'}">판매자의
						다른 상품 보기</a> <br> <a href="#">판매자 후기 작성하기</a>
				</div>
			</aside>
		</section>
		...
		<section class="comment-write-section section">
			...
			<form action="/product/insertComment" method="post"
				...
				<div class="comment-input-wrap">
					...
					<!-- 댓글 입력창 -->
					<textarea class="comment-textarea" name="commentContent"
						maxlength="500"
...
거래와 무관한 광고성 댓글은 금지되며, 신고 대상이 될 수 있습니다."></textarea>
					...
					<div class="comment-submit-wrap">
						<div class="btn-wrap">
							<span class="comment-char-count"><span
								class="current-count">0</span> / 500</span>
		...
		<section class="comment-box-section section">
			<div class="comment-box-header">
				<h3 class="section-title">
					전체 댓글 (<span>${fn:length(commentList)}</span>개)
				</h3>
				<div class="comment-box-sort">
					정렬 : <strong>최신순</strong>
				</div>
			</div>
			<!-- 댓글이 0개일 경우 나오는 화면 -->
			<c:if test="${empty commentList}">
				<div class="comment-empty-box">현재 등록된 댓글은 0개입니다.</div>
			</c:if>
				...
				<c:if test="${empty comment.parentCommentNo}">
					...
					<div class="comment-box-item">
						...
						<div class="comment-box-body">
							...
							<div class="comment-box-footer">
								<span class="comment-box-date"> <fmt:formatDate
										value="${comment.createdDate}" pattern="yyyy-MM-dd" />
								</span>
								<div class="comment-box-actions">
									...
									<c:if
										test="${loginMember.memberNo == comment.memberNo or loginMember.memberNickname eq 'admin'}">
										<button class="comment-box-delete-btn"
											onclick="deleteComment(${comment.commentNo}, '${comment.productNo}')">
											삭제</button>
									</c:if>
									<button class="comment-box-reply-btn"
										onclick="openReplyModal(${comment.commentNo})">답글</button>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="comment-box-replies">
						<c:forEach var="reply" items="${commentList}">
							<c:if
								test="${not empty reply.parentCommentNo and reply.parentCommentNo eq comment.commentNo}">
								<div class="comment-box-item comment-reply">
									...
									<div class="comment-box-body">
										...
										<div class="comment-box-footer">
											<span class="comment-box-date"> <fmt:formatDate
													value="${reply.createdDate}" pattern="yyyy-MM-dd" />
											</span>
											<div class="comment-box-actions">
												...
												<c:if
													test="${loginMember.memberNo == reply.memberNo or loginMember.memberNickname eq 'admin'}">
													<button class="comment-box-delete-btn"
														onclick="deleteComment(${reply.commentNo}, '${reply.productNo}')">
														삭제</button>
												</c:if>
											</div>
										</div>
		...
		<!-- 대댓글 작성 모달 -->
		<div id="replyModal" class="modal-overlay" role="dialog"
			aria-modal="true"
			...
			<div class="modal-content"
				...
				<form action="/product/insertComment" method="post"
					...
					<input type="hidden" name="commentWriter"
						...
						name="parentCommentNo" id="modalParentCommentNo" />

					<!-- 대댓글 입력창 -->
					<textarea name="commentContent" class="comment-textarea"
						maxlength="500" placeholder="대댓글 내용을 입력해주세요."
					...
					<div class="comment-submit-wrap" style="margin-top: 14px;">
						...
						<button type="button" class="btn-secondary sm"
							onclick="closeReplyModal()">취소</button>
					</div>
				</form>
			</div>
		</div>
		...
		<div id="editModal" class="modal-overlay"
			...
			<div class="modal-content"
				...
				<form action="/product/updateComment" method="post"
					class="reply-form">
					<!-- 댓글 번호 전달 -->
					...
					<!-- 수정 textarea -->
					<textarea name="commentContent" id="modalEditContent"
						class="comment-textarea" maxlength="500"
...
<script>
  function updateReportCharCount() {
    ...
    const counter = document.getElementById("reportDetailCount");
    if (textarea && counter) {
        if (textarea.value.length > 100) {
          textarea.value = textarea.value.slice(0, 100); // 50자 초과 시 잘라냄
        }
        counter.textContent = textarea.value.length + " / 100";
      }
    }

  // JSTL에서 전달한 로그인 여부 변수 (true/false로 변환됨)
  const isLoggedIn = ${not empty sessionScope.loginMember};

  // 신고 모달 열기 함수 (로그인 검사 포함)
  function openReportModal() {
    ...
    // DOM 렌더링 완료 이후에 안전하게 요소를 다룸
    requestAnimationFrame(() => {
      const textarea = document.getElementById("reportDetail");
      const counter = document.getElementById("reportDetailCount");

      if (textarea && counter) {
        textarea.removeEventListener("input", updateReportCharCount); // 중복 방지
        textarea.addEventListener("input", updateReportCharCount);
        updateReportCharCount();  // 글자 수 초기화 표시
      }
    });
  }
