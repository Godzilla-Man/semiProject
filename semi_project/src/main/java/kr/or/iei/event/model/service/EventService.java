...
public class EventService {
	...
	public ListData<Event> selectEventList(int reqPage) {
		 ...
		 */
		ListData<Event> listData = new ListData<>();
	...
	//이벤트 글 등록
	public int insertEvent(Event event, List<Files> fileList) {
		Connection conn = JDBCTemplate.getConnection();
		...
		if(result > 0) {
			for(Files file : fileList) {
				...
				result = dao.insertEventFile(conn, event, fileList);
	...
	public int updateEvent(Event event, List<Files> fileList) {
		Connection conn = JDBCTemplate.getConnection();

		//1)기존 파일 삭제
		int dResult = dao.deleteEventFile(conn, event);

		//2) 이벤트 내용 업데이트
		int uResult = dao.updateEvent(conn, event);
		if(uResult > 0) {
			//3) 파일 업데이트
			int fResult = dao.insertEventFile(conn, event, fileList);
	...
	public List<Files> deleteEvent(String eventNo) {
		Connection conn = JDBCTemplate.getConnection();

		//서버에 있는 파일 삭제
		List<Files> delFileList = dao.selectEventFileList(conn, eventNo);
	...
	//파일 정보
	public List<Files> selectEventFileList(String eventNo) {
		Connection conn = JDBCTemplate.getConnection();
		List<Files> fileList = dao.selectEventFileList(conn, eventNo);
		JDBCTemplate.close(conn);
		return fileList;
	}

	//index에 사용될 파일 정보
	public List<Files> selectFirstEventFileList() {
		Connection conn = JDBCTemplate.getConnection();
		List<Files> fileList = dao.selectFirstEventFileList(conn);
		JDBCTemplate.close(conn);
		return fileList;
	}
}
